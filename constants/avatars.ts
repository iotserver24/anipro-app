...
export const getAvatarById = async (avatarId: string): Promise<string> => {
  ...
  // Custom avatars (ID starts with custom_) should use avatarUrl from user document
  // Return empty string to signal the caller to use the stored URL
  if (avatarId.startsWith('custom_')) {
    logger.debug('Avatars', `Custom avatar detected: ${avatarId}, caller should use avatarUrl`);
    return '';
  }
